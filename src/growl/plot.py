...
from .constants import StellarType
...
def plot_stellar_masses(df: pd.DataFrame, events: pd.DataFrame):
    ...
    for _, ev in events[events.event_types.map({"mt_history"}.issubset)].iterrows():
        ax.axvline(ev.time_f, color="grey", lw=1, ls="-", alpha=0.5, zorder=0)
...
def plot_stellar_radii(df: pd.DataFrame, events: pd.DataFrame):
    del events
...
def plot_orbit_parameters(df: pd.DataFrame, events: pd.DataFrame):
    ...
    for ax in axes:
        for _, ev in events[events.event_types.map({"mt_history"}.issubset)].iterrows():
            ax.axvline(ev.time_f, color="grey", lw=1, alpha=0.6, zorder=0)
    ...
    ce_start = np.where(df["mt_history"].values == 4)[0][0]  # type: ignore
...
def plot_hr_diagram(df: pd.DataFrame, events: pd.DataFrame):
    del events
    ...
    for star, colour, name in [(1, C1, "Star 1"), (2, C2, "Star 2")]:
        is_star = df[f"stellar_type_{star}"] <= 9
        d = df[is_star]
        ax.plot(d[f"teff_{star}"], d[f"luminosity_{star}"], color=colour, lw=1.8, label=name)
        ax.plot(
            d[f"teff_{star}"].iloc[0],
            d[f"luminosity_{star}"].iloc[0],
            "o",
            color=colour,
            ms=9,
            mec="white",
        ...
        ax.plot(
            d[f"teff_{star}"].iloc[-1],
            d[f"luminosity_{star}"].iloc[-1],
            "*",
            color=colour,
            ms=17,
            mec="white",
...
def plot_summary(df: pd.DataFrame, events: pd.DataFrame):
    ...
    print(
        f"  Star 1: {StellarType(df['stellar_type_1'].iloc[-1])}, {df['mass_1'].iloc[-1]:.2f} Msun"
    )
    print(
